...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| project-os | `C:\Users\JohnKim\Documents\Codex\2026-05-08\repo-github-codex-chatgpt-source-of` | operating/status repo | No app code | Status checks only | Yes, branch `codex/heater-onboarding-status` | `https://github.com/BohyungKim/project-os.git` | `project-os` | Established; PR #4 merged to `main` | Review PR #5 after validation |
...
| heater-batch-selection | `C:\Users\JohnKim\Documents\New project 2` | real app/code project | Yes, Python package under `src/heater_batch` | Yes, `python -m pytest` passed 9 tests | Yes, branch `main`, remote commit `6e32db7` | `https://github.com/BohyungKim/heater-batch-selection.git` | `heater-batch-selection` | Established; GitHub `main` is the source of truth | Onboard `prg-supply-readiness-checker` next |
...
2. `heater-batch-selection`
   ...
   - Latest remote `main` commit: `6e32db7 docs: confirm heater GitHub source of truth`.
   - `origin` is configured and GitHub `main` is established as the source of truth.
...
- `New project 2` now has a GitHub remote and `origin/main` is the stable source of truth.
...
Onboard `C:\Users\JohnKim\Documents\New project 3` as `BohyungKim/prg-supply-readiness-checker` as a docs-first repo.
